...
public class Upserts_Spans {
    ...
    public async Task Conditional_Update_TArg_When_Not_Found_Inserts() {
        ...
        Assert.True(db.Upsert(key, 1, JContext.Default.Int32, static (reference, digit) => reference == int.Parse(digit), "1"));
    }
    ...
    public async Task Conditional_Update_TArg_When_Found_And_Valid_Updates() {
        ...
        db.Upsert(key, 1, JContext.Default.Int32);
        // using a static delegate ensures that closure cannot be allocated
        Assert.True(db.Upsert(key, 3, JContext.Default.Int32, static (reference, digit) => reference == int.Parse(digit), "1"));
        Assert.True(db.TryGetValue(key, JContext.Default.Int32, out var value));
        Assert.Equal(3, value);
    }
    ...
    public async Task Conditional_Update_TArg_When_Found_And_Invalid_Returns_False() {
        ...
        db.Upsert(key, 1, JContext.Default.Int32);
        // using a static delegate ensures that closure cannot be allocated
        Assert.False(db.Upsert(key, 3, JContext.Default.Int32, static (reference, digit) => reference == int.Parse(digit), "3"));
        Assert.True(db.TryGetValue(key, JContext.Default.Int32, out var value));
        Assert.Equal(1, value);
    }
}